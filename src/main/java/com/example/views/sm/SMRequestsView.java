...
public class SMRequestsView extends VerticalLayout implements BeforeEnterObserver {
    ...
    private final ComboBox<WorkflowStatus> statusFilter = new ComboBox<>("Akış Durumu");
    ...
    public SMRequestsView(PrioritizationService prioritizationService, CompanyService companyService) {
        ...
        incomingGrid.addColumn(w -> (w.getRequest() != null && w.getRequest().getCustomer() != null) ? 
                w.getRequest().getCustomer().getName() : "-").setHeader("Müşteri").setSortable(true).setFlexGrow(1);
        ...
        incomingGrid.addComponentColumn(w -> {
        ...
        incomingGrid.addColumn(Workflow::getStatus).setHeader("Akış Durumu").setSortable(true).setAutoWidth(true);

        incomingGrid.addComponentColumn(workflow -> {
            Button evaluateBtn = new Button("Değerlendir", VaadinIcon.TOOLS.create());
            evaluateBtn.addThemeVariants(ButtonVariant.LUMO_PRIMARY, ButtonVariant.LUMO_SMALL);
            ...
            return evaluateBtn;
        }).setHeader("İşlem").setWidth("140px").setFlexGrow(0);

        incomingGrid.setWidth("calc(100% - 40px)");
        incomingGrid.getStyle()
                .set("margin", "0 auto")
        ...
        add(headerBanner, filterCard, incomingGrid);
    }
    ...
    private VerticalLayout createFilterBar() {
        ...
        HorizontalLayout row1 = new HorizontalLayout(searchField, companyFilter, statusFilter, financialImpactFilter, securityFilter);
        row1.setWidthFull();
        row1.setSpacing(true);

        HorizontalLayout row2 = new HorizontalLayout(createdStartFilter, createdEndFilter, deadlineStartFilter, deadlineEndFilter);
        row2.setWidthFull();
        ...
        filterFieldsContainer.add(row1, row2, clearRow);
    ...
    private void applyFilters() {
        ...
        List<Workflow> filtered = allDepartmentWorkflows.stream().filter(w -> {
            boolean matchesQuery = query.isEmpty() ||
                    (w.getRequest() != null && String.valueOf(w.getRequest().getId()).contains(query)) ||
                    (w.getRequest() != null && w.getRequest().getTitle() != null && w.getRequest().getTitle().toLowerCase().contains(query)) ||
                    (w.getRequest() != null && w.getRequest().getDescription() != null && w.getRequest().getDescription().toLowerCase().contains(query)) ||
                    (w.getRequest() != null && w.getRequest().getCustomer() != null && w.getRequest().getCustomer().getName() != null && w.getRequest().getCustomer().getName().toLowerCase().contains(query));
            ...
            return matchesQuery && matchesCompany && matchesStatus && matchesFinancial && matchesSecurity && matchesCreated && matchesDeadline;
        }).toList();

        incomingGrid.setItems(filtered);
    }
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        if (currentUser == null || currentUser.getDepartment() == null) {
            allDepartmentWorkflows = Collections.emptyList();
            incomingGrid.setItems(allDepartmentWorkflows);
            return;
        }
        
        Long smDepartmentId = currentUser.getDepartment().getId();
        allDepartmentWorkflows = prioritizationService.getWorkflowsForDepartment(smDepartmentId);
        applyFilters();
    }
